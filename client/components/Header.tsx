    ...
    <header className="sticky top-0 z-40 border-b border-white/5 bg-surface/70 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          <Link href="/cart" className="relative p-2 text-ink-muted transition hover:text-ink">
          ...
          {isLoggedIn ? (
            <>
              <Link href="/orders" className="hidden text-sm text-ink-muted transition hover:text-ink sm:block">
                Orders
              </Link>
              {user?.role === 'admin' && (
                <Link href="/admin" className="hidden text-sm text-ink-muted transition hover:text-ink sm:block">
                  Admin
                </Link>
              )}
